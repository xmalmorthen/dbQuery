...
class Fire_log{
	...
	public function __construct()
	{
		...
		public function filter( $str )
		{
			...
			$regex = "/(INFO|DEBUG|ERROR)\s{1}-{1}\s{1}(.*)/im";
			if ($this->url_vals['filter'] != 'all')
				$regex = "/(" . $this->url_vals['filter'] . ")\s{1}-{1}\s{1}(.*)/im";

			preg_match_all($regex, $content, $match, PREG_PATTERN_ORDER);
			$content = '';
			foreach ($match[0] as $key => $value) {
				$content .= '<div class="' . strtolower($match[1][$key]) .'">' . $value . '</div>';
			}
